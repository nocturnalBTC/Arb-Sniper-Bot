...
Co='SELL ALL'
Cn='SELL NOW'
Cm='There are no tokens to be sold!'
Cl='Sell all function initiated - Stopping operation'
Ck='SL Hit!'
Cj='TP Hit!'
Ci='Securing SL to '
Ch=' | SL: '
...
D={}
A_={}
Ct=BU('ck_258b79c41004f53e58d0e5fa11486361bdcace02','cs_bd6506935df71db41cf1e545188f1f9ae2306134')
BX=BV.now()
Cu=CU
Cv=BX.strftime(CU)
def BY():
	...
	A_[AR]='https://arb1.arbitrum.io/rpc';A(f,BW,A_)
def BZ():
	def A(path2,file_name,data2):
		A=r+path2+A4+file_name
		with L(A,A5)as B:e.dump(data2,B,indent=2)
...
A9=C.toChecksumAddress('0x82aF49447D8a07e3bd95BD0d56f35241523fBab1')
g=C.toChecksumAddress('0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8')
U=d(L(BM+'erc20.abi'))
V=C.eth.contract(address=R.toChecksumAddress('0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506'),abi=d(L(BM+'router.abi')))
BC=C.eth.contract(address=R.toChecksumAddress('0xc35DADB65012eC5796536bD9864eD8773aBc74C4'),abi=d(L(BM+'factory.abi')))
...
def Bl():
	...
	while Q:
		...
		if B!=CZ:
			...
			if D!=0:A(Ca,Cb);A(Cc+B);A(Cd+A3(C.fromWei(D,c))+' ETH');Bg();break
			else:A8(5);A(Au,J)
		else:A8(5);A(Au,J)
...
def Bp():
	...
	while Q:
		try:
			N=M.functions.balanceOf(G).call()-1;F=I(C.fromWei(V.functions.getAmountsOut(N,[H,A9]).call()[-1],c));D=Ap(I(F)/I(q)*100,5);A('ETH Value Now: {} / '.format('%.5f'%F)+Cg.format(D)+Ch+A3(B)+'%')
			if E!=0:
				if D-E>=B:B=D-E;A(Ci+A3(B))
			A8(2)
		except:continue
		try:
			if I(D)>=I(K):A(Cj,O);w();h();break
			if I(D)<=I(B):A(Ck,J);w();h();break
			if Y:Y=A6;A(Cl,b);w();h();break
		except CR:A(Cm,J);break
Bq='gAAAAABh7KbIGnFCH7Gp_4OK-vW0v-2ZNTkzqFB8k4xmk4aV4_n-TxZEsE361BfcfNjRwTZ8nVTAp6ZBXoDXRaQgUpyXfUzVyQ=='
def Br():
	global Y;BD();i();K=I(AO);L=I(AP);B=L;E=I(AQ);M=C.eth.contract(address=H,abi=U);A(Cf,b)
	while Q:
		try:
			N=M.functions.balanceOf(G).call()-1;F=I(C.fromWei(V.functions.getAmountsOut(N,[H,g]).call()[-1],c));D=Ap(I(F)/I(q)*100,5);A('USDC Value Now: {} / '.format('%.3f'%F)+Cg.format(D)+Ch+A3(B)+'%')
			if E!=0:
				if D-E>=B:B=D-E;A(Ci+A3(B))
			A8(2)
		except:continue
...
def BD():Ao.place_forget();A=E.Button(B.widgets_frame,text=Cn,command=BF,style=Av);A.grid(row=18,column=5,padx=10,pady=(0,10),sticky=F,rowspan=1)
def w():CQ.place_forget();A=E.Button(B.widgets_frame,text=Co,command=BE);A.grid(row=18,column=5,padx=10,pady=(0,10),sticky=F,rowspan=1)
def Bt():
...
def B_():
	if B.tk.call('ttk::style','theme','use')=='sun-valley-dark':B.tk.call(BP,Cp);AM[BQ].configure(bg=Ay)
	else:B.tk.call(BP,'dark');AM[BQ].configure(bg='black')
B=P.Tk()
B.title('ARBITRUM Sniper Bot - V1')
B.geometry('1050x730')
...
Ak=E.Separator(B,orient=Cq)
C9=C0+C8+C2+C5
Ak.place(x=10,y=135,width=625)
def CA():X.delete(0,Cr);X.insert(0,BR.paste());return
def CB():X.delete(0,Cr);return
def CC():
	if AF!=N:
		try:A=BT(C6,C9+AF)
		except AC:pass
...
X.insert(0,B2)
AJ.insert(0,Be)
Ak=E.Separator(B.widgets_frame,orient=Cq)
...
CG=E.Label(B.widgets_frame,text='Wallet ETH:')
CG.grid(row=7,column=4,padx=10,pady=(0,10),sticky=F,rowspan=1)
Al=E.Label(B.widgets_frame,width=12,relief=Cs)
...
Am=E.Label(B.widgets_frame,width=12,relief=Cs)
...
CQ=E.Button(B.widgets_frame,text=Cn,command=BF,style=Av)
Ao=E.Button(B.widgets_frame,text=Co,command=BE)
